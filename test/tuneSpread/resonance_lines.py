...
class resonance_lines(object):
	...
	# def plot_resonance(self, figure_object = None, interactive=False):	
	# 	if(interactive): 
	# 		plt.ion()
	# 	if figure_object:
	# 		fig = figure_object
	...
	def plot_resonance(self, ax=None, interactive=False):    
		if interactive: 
			plt.ion()
		...
		ax.set_xlabel('$\mathrm{Q_xx}$', fontsize='large')
		ax.set_ylabel('$\mathrm{Q_y}$', fontsize='large')

		for resonance in self.resonance_list:
			nx, ny, res_sums = resonance
			for res_sum in res_sums:
				if ny != 0:
					x_vals = np.array([self.Qx_min, self.Qx_max])
					y_vals = (res_sum - nx * x_vals) / ny
					line, = ax.plot(x_vals, y_vals)
				else:
					x = res_sum / nx if nx != 0 else 0
					line, = ax.plot([x, x], [self.Qy_min, self.Qy_max])

				if ny % 2:
					line.set_linestyle('--')  # skew resonances
				if res_sum % self.periodicity:
					line.set_color('b')  # non-systematic
					line.set_zorder(1)
				else:
					line.set_color('r')  # systematic
